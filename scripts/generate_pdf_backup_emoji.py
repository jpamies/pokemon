...
def draw_pokemon_card(c, pokemon, image, x, y, card_width, card_height):
    ...
    special_y = types_y - (len(pokemon['types']) * 22) - 5
    
    if pokemon.get('is_legendary', False):
        # Legendary badge
        c.setFillColor(HexColor('#B8860B'))  # Dark gold (más oscuro que eléctrico)
        c.roundRect(right_x, special_y, 70, 18, 9, fill=1)
        c.setFillColor(HexColor('#ffffff'))  # White text for better contrast
        c.setFont("Helvetica-Bold", 8)
    ...
    if pokemon.get('is_mythical', False):
        # Mythical badge
        c.setFillColor(HexColor('#C0C0C0'))  # Silver
        c.roundRect(right_x, special_y, 70, 18, 9, fill=1)
