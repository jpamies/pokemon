#!/usr/bin/env python3
"""
Script para traducir todas las descripciones de Pokémon del inglés al catalán
"""
...
def translate_to_catalan(english_text):
    """Traduce texto del inglés al catalán"""
    
    # Diccionario de traducciones específicas de Pokémon
    pokemon_terms = {
        'POKéMON': 'Pokémon',
        'POKEMON': 'Pokémon',
        'Pokemon': 'Pokémon',
    ...
    # Traducciones directas palabra por palabra
    translations = {
        # Descripciones completas más comunes
    ...
    # Aplicar traducciones específicas de términos Pokémon
    text = english_text
    for eng_term, cat_term in pokemon_terms.items():
        text = text.replace(eng_term, cat_term)
    
    # Buscar traducción completa
...
def main():
    ...
    print("Traduciendo descripciones al catalán...")
    ...
    for pokemon_id, pokemon_info in pokemon_data.items():
        ...
        if int(pokemon_id) % 100 == 0:
            print(f"Procesado Pokémon #{pokemon_id}")
    
    # Guardar traducciones
    output_file = 'data/catalan_translations_new.json'
    os.makedirs('data', exist_ok=True)
    
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(catalan_translations, f, ensure_ascii=False, indent=2)
    
    print(f"\n✅ Traducciones guardadas en: {output_file}")
