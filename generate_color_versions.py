#!/usr/bin/env python3
"""
Generate color-sorted PDFs for generations VI-IX
"""
...
def generate_generation_by_color(start_id, end_id, filename_prefix, title):
    """Generate PDF for specific generation range sorted by color"""
    print(f"Generating {title} - Por Color...")
    ...
    # Sort by color
    def get_pokemon_color(pokemon):
        if 'species_data' in pokemon and 'color' in pokemon['species_data']:
            return pokemon['species_data']['color']['name']
        return 'white'
    
    pokemon_list.sort(key=get_pokemon_color)
    
    filename = f"pdf/{filename_prefix}_by_color.pdf"
    ...
    c.drawString(width/2 - 150, height - 100, f"{title} - Per Color")
    c.showPage()
    ...
    c.save()
    print(f"Generated {filename} with {total_pokemon} Pokemon sorted by color")

if __name__ == "__main__":
    # Generate color versions for missing generations
    generations = [
    ...
    for start_id, end_id, gen_name, gen_title in generations:
        generate_generation_by_color(start_id, end_id, gen_name, gen_title)
        print()
